...
## Status: ✅ **W3 + W4 + W5 complete · W6 polish in progress** — semantic search + auto-tag + AI weekly digest with LLM hooks + deep-indexed code search across starred repos all wired through popup. W6 D1 R10 蓝军 残留 fixes (digest stale-closure cancellation, rate-limit cooldown UX, service-worker log gating) landed. W6 D2 store-submission docs at `docs/STORE_SUBMISSION.md`. 412 tests green, two end-to-end demo-gate smokes passing, CI gated. **Next: build icon set + screenshots + version bump to v0.1.0 → store submission.**
...
- **W5 ✅** — D1 regex-based code chunker (`@starkit/core/code/chunk`: semantic per-language splits for JS/TS/Python/Rust/Go/Java, sliding-window fallback, ChunkKind discriminator preserved across splits for downstream weighting), D2 source fetcher + deep-index orchestrator (`fetchRepoSource`: 1 tree call + ≤20 selected-file content calls per opt-in, extension whitelist + path-segment deny + 100KB cap; `indexRepoCode`: starStore → chunkSource per file → batched embed → upsert with `code:{repoId}:{path}:{idx}` namespace; metadata.snippet/path/startLine/endLine stamped for zero-refetch search rendering), D3 popup wiring ("🔧 Deep-index top 3" button picks top stargazers-count unindexed/non-archived/non-fork; CodeSnippet renders with permalink to `{htmlUrl}/blob/{defaultBranch}/{path}#L{a}-L{b}`; mixed star + code SearchHit discriminated union; search limit 5→8 for headroom), D4 demo-gate smoke (end-to-end debounce-query → top code hit with correct headerLine + metadata, 13ms hop). R10 蓝军 pre-W5 audit caught: bundle bloat from createProvider factory (→ direct OpenAIProvider import saves ~6KB), unused contextMenus permission + overbroad github.com host_permission (→ removed for store-reviewer hygiene), missing v1→v2 schema migration test (→ added 4-test regression suite that proves additive upgrade preserves stars/kv/cursor), sync.lock not cleared on reset (→ added chrome.storage.local.remove to onClearAll).
- **W6 🚧** — D1 R10 蓝军 残留 🟡 fixes: (i) digest stale-closure cancellation via `digestGenRef` generation counter — onShowDigest / onCloseDigest / onClearAll all bump it so a slow ~3s `summarizeDigestEntries` can no longer write back state over a newer view; (ii) rate-limit recovery UX — onSync catches `GithubError.kind === 'rate_limit'` and persists `rateLimitResetAt`; the Sync button stays disabled with a "Wait Nm" label until the cooldown clears; (iii) service-worker log gating behind `import.meta.env.DEV` so production users see no console chatter (errors + warnings still print). R10-5 (Float32Array memory halving for 5000+ star accounts) deferred to v0.2 — invasive refactor across `@starkit/vector` public type contract. D2 store-submission checklist + privacy disclosures at `docs/STORE_SUBMISSION.md`; README rewritten with a step-by-step "How to use" so first-time installers from the Chrome Web Store can self-onboard.
